    ...
    return (
        <section className="mx-8 mt-8 md:mx-16 lg:mx-24" id="about">
            ...
            <div className="bg-white rounded-lg w-full shadow-md mt-8">
                ...
                <button onClick={toggleReadMore} className={`2xl:hidden px-4 underline p-1 rounded-md ${isExpanded ? 'bg-neutral-200' : 'bg-black text-white'}`}>{isExpanded ? 'Ler menos' : 'Ler mais'}</button>
            </div>
        </section>
    )
}